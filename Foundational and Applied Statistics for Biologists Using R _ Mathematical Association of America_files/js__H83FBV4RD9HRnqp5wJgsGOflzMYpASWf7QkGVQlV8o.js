...
// modify these
var enl_gifpath='/sites/default/files/images/pubs/books/covers/'; // path to graphics
...
(function($){
    ...
    //this is related to the ssl login issues
    //see nvl_modals.js file for more information
		...
		if (getParameter('triggerlogin') == 'true') {
		    $('#block-menu-menu-utility-menu li').each(function(){
    ...
    //end of code related to ssl login issues



		// Publications tabs
		$("#book-tabs").tabs();
		// console.log('hasNinjas') // test

		// Front date size
		$('.views-field-field-news-date .date-display-single').each(function(){
		...
		// Loci Front-End Changes
		if($('body').hasClass('node-type-loci-article')){
			$('.book_add_child a').attr('href', $('.book_add_child a').attr('href').replace('/node/add/book?parent=', '/node/add/loci-article?parent='));
			$('.field-name-field-authors .field-label').text('By').append('&nbsp;');
			if($('.field-name-field-authors .field-item').text() == ""){
				$('.field-name-field-authors').css('display', 'none');
			}			
		}
		
		// Course Community Front-End Changes
		if($('body').hasClass('node-type-course-community')){

			if($('.field-name-field-related-resources .field-items').length > 0){
				$html = $('.field-name-field-related-resources .field-items').html();
				$text = $html.replace(/div/gi, 'li');
				$('.field-name-field-related-resources .field-items').html($text);
			}

			if($('.field-name-field-identifier .field-item').length > 0){
				$identifier = '<a href="' + $('.field-name-field-identifier .field-item').text() + '" target="_blank">' + $('.field-name-field-identifier .field-item').text() + "</a>";
				$('.field-name-field-identifier .field-item').html($identifier);
			}			
		}

		// Scroll anchors
		$(".maa-scroll").click(function(event){
